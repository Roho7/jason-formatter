...
import { saveJsonEntry, getLatestJsonEntry, handlePasteEvent } from '../_utils/utils';
...
const JsonDiff = () => {
  ...
  useEffect(() => {
    const latestLeftEntry = getLatestJsonEntry('diff-left');
    const latestRightEntry = getLatestJsonEntry('diff-right');
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const rightEditor = document.querySelector('[data-testid="right-editor"]');
      
      if (leftEditor && leftEditor.contains(activeElement)) {
        handlePasteEvent(event, leftJson, 'diff-left', () => {
          console.log('Left JSON saved to history!');
        });
      } else if (rightEditor && rightEditor.contains(activeElement)) {
        handlePasteEvent(event, rightJson, 'diff-right', () => {
          console.log('Right JSON saved to history!');
        });
      }
    };
  ...
  const { leftHighlights, rightHighlights, diffResults } = useMemo(() => {
    ...
    try {
      ...
      const compareObjects = (left: any, right: any, path: string[] = []) => {
        const leftKeys = left && typeof left === 'object' && !Array.isArray(left) ? Object.keys(left) : [];
        const rightKeys = right && typeof right === 'object' && !Array.isArray(right) ? Object.keys(right) : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div data-testid="left-editor">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-1 rounded ${leftValid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {leftValid ? 'Valid' : 'Invalid'}
              </span>
              {leftHighlights.length > 0 && (
                <span className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-800">
                  {leftHighlights.length} line{leftHighlights.length !== 1 ? 's' : ''} highlighted
                </span>
              )}
            </div>
          </div>
        ...
        <div data-testid="right-editor">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-1 rounded ${rightValid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {rightValid ? 'Valid' : 'Invalid'}
              </span>
              {rightHighlights.length > 0 && (
                <span className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-800">
                  {rightHighlights.length} line{rightHighlights.length !== 1 ? 's' : ''} highlighted
                </span>
              )}
            </div>
          </div>
      ...
      {diffResults && (
        <div className="bg-gray-50 rounded-lg p-4">
          <h4 className="font-semibold mb-3 text-gray-800">Differences Found:</h4>
          {diffResults.length === 0 ? (
            <p className="text-green-600">No differences found - JSONs are identical!</p>
          ) : (
              ...
              {diffResults.map((diff, index) => (
                <div key={index} className="bg-white rounded-lg border overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 p-3 bg-gray-50 border-b">
                    ...
                    <span className="font-mono text-gray-700 font-medium">{diff.key}</span>
                  </div>
                      ...
                      <>
                        ...
                        <div className="flex bg-red-50 border-l-4 border-red-400">
                          ...
                          <div className="flex-1 px-3 py-2 text-red-800">
                            <span className="text-red-600">-</span> {diff.leftLineContent}
                          </div>
                        </div>
                        {/* Added line (green) */}
                        <div className="flex bg-green-50 border-l-4 border-green-400">
                          ...
                          <div className="flex-1 px-3 py-2 text-green-800">
                            <span className="text-green-600">+</span> {diff.rightLineContent}
                          </div>
                        </div>
                      </>
                    )}
